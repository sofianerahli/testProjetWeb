<?php
require_once("header.php");

session_start();

if(isset($_POST['submit'])) 
{
	if(($_POST['username'])&&($_POST['email'])&&($_POST['nom']))
	{
		$username=$_POST['username'];
		$_SESSION['username']=$_POST['username'];
		$email=$_POST['email'];
		$nom=$_POST['nom'];
		$database = "ece_amazon";
		$db_handle = mysqli_connect('localhost', 'root', '' );
		$db_found = mysqli_select_db($db_handle, $database );
		if($db_found)
		{
			$SQL="SELECT * FROM vendeur WHERE (pseudo='$username')";
			$result = mysqli_query($db_handle, $SQL);
			$donnee = mysqli_fetch_array($result);
     		
     		if($donnee)
			{
				?>
    			<script type="text/javascript"> alert("Pseudo déjà pris");</script>
    			<?php
			}
			else
			{
				$SQL2 = "INSERT INTO vendeur( email, pseudo, nom)  VALUES ('$email', '$username', '$nom')";
     			$result2 = mysqli_query($db_handle, $SQL2);
     			

				header('Location:choixfond.php');			
			}
		
			mysqli_close($db_handle);
		}
	}
		

		
	
	
		
	
	else
	{
		?>
    	<script type="text/javascript"> alert("Champs vides: Veuillez remplir");</script>
    	<?php
	}

}
?>


<div style="background:#A9F5E1;width:35% ; padding: 0.5%;
    margin-left: auto;
    margin-right: auto;
    border: 1px solid #f1f1f1;
    box-shadow: 0 0 20px 0 rgba(0, 0, 0, 0.2), 0 5px 5px 0 rgba(0, 0, 0, 0.24);box-sizing: border-box;">
<body style="background-image: url('compteAcheteur.jpg'); background-size:cover;  "></body>
<form align="center" action="" method="POST">
<h1><u><big>Création du compte</big></u></h1>
<h3>Pseudo :</h3><input placeholder="Entrez un pseudo" type="text" name="username"/><br/><br/>
<h3>Votre adresse mail :</h3>
<input placeholder="Entrez un mail" type="email" name="email"/><br/></br/>
<h3>Votre Nom :</h3>
<input placeholder="Entrez un nom" type="text" name="nom"/><br/></br/>
<input type="submit" name="submit"/><br/><br/>
</form>
</div>

<?php
echo
require_once("footer.php");
?>